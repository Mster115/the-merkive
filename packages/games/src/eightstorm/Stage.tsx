...
export function EightstormStage({ room, match, t }: StageProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-[1800px] mx-auto p-4 md:p-8 justify-between select-none">
      ...
      <div className="flex items-center justify-between gap-4 bg-[var(--mb-surface)] p-4 rounded-xl border-2 border-black shadow-[var(--mb-shadow)]">
        <div className="flex items-center gap-3">
          <Pill tone="accent" className="text-base px-4 py-1.5 font-black">
            {t("games.eightstorm.name")}
          </Pill>
          <span className="font-extrabold text-lg text-[var(--mb-text-dim)] flex items-center gap-2">
            <ReverseIcon
              className={cn(
                "w-5 h-5 text-[var(--mb-violet)] transition-transform duration-500",
        ...
        {pub.pendingDraw > 0 && (
          <Pill tone="danger" className="mb-blink text-base px-5 py-1.5 font-black border-2 border-black shadow-[var(--mb-shadow)] flex items-center gap-1.5">
            <LightningIcon className="w-5 h-5 text-[var(--mb-on-danger)]" />
        ...
        {pub.declaredSuit && (
          <div className="flex items-center gap-2">
            <span className="text-sm font-black uppercase text-[var(--mb-text-dim)] [font-family:var(--mb-font-display)]">
              {t("games.eightstorm.ui.active_suit")}:
            </span>
            <span className="px-4 py-1 rounded-xl border-2 border-black bg-[var(--mb-gold)] text-[var(--mb-on-gold)] font-black text-lg flex items-center gap-1.5 shadow-[var(--mb-shadow)]">
              <SuitSVG suit={pub.declaredSuit} className="w-5 h-5 text-[var(--mb-on-gold)]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center my-auto">
        ...
        <div className="flex flex-col items-center justify-center">
          <div className="relative group cursor-pointer">
            ...
            <div className="absolute top-2 left-2 w-36 h-52 rounded-2xl bg-[var(--mb-surface-3)] border-2 border-black shadow-[var(--mb-shadow)] transform -rotate-6" />
            <div className="absolute top-1 left-1 w-36 h-52 rounded-2xl bg-[var(--mb-surface-2)] border-2 border-black shadow-[var(--mb-shadow)] transform rotate-3" />

            {/* Top Deck Card */}
            <Panel className="w-36 h-52 rounded-2xl flex flex-col items-center justify-center border-[3px] border-black bg-[var(--mb-surface)] shadow-[var(--mb-shadow-lg)] relative z-10">
              <div className="w-28 h-44 rounded-xl border-2 border-dashed border-[var(--mb-line-dim)] bg-[var(--mb-surface-2)] flex flex-col items-center justify-center p-2">
                ...
                <span className="font-black text-3xl text-[var(--mb-text)] tracking-wider [font-family:var(--mb-font-display)]">
                  {pub.drawPileCount}
                </span>
                <span className="text-[0.65rem] uppercase font-black text-[var(--mb-text-dim)] tracking-widest mt-1 [font-family:var(--mb-font-display)]">
                  Remaining
                </span>
              </div>
            </Panel>
        ...
        <div className="flex flex-col items-center justify-center relative">
          <div className="relative flex flex-col items-center">
            ...
            <div className="absolute top-1 left-2 w-32 h-48 rounded-2xl bg-[var(--mb-surface-3)] border-2 border-black shadow-[var(--mb-shadow)] transform rotate-6" />
            <div className="absolute -top-1 -left-1 w-32 h-48 rounded-2xl bg-[var(--mb-surface-2)] border-2 border-black shadow-[var(--mb-shadow)] transform -rotate-3" />
            ...
            {pub.declaredSuit && (
              <div className="absolute -top-4 -right-4 px-4 py-1.5 rounded-xl bg-[var(--mb-gold)] text-[var(--mb-on-gold)] font-black text-base shadow-[var(--mb-shadow)] border-2 border-black z-20 flex items-center gap-1.5 rotate-3">
                <SuitSVG suit={pub.declaredSuit} className="w-5 h-5 text-[var(--mb-on-gold)]" />
                {SUIT_NAMES[pub.declaredSuit]}
              </div>
            )}
          </div>
          <span className="mt-4 font-black text-sm uppercase tracking-widest text-[var(--mb-text-dim)] [font-family:var(--mb-font-display)]">
        ...
        <div className="flex flex-col items-center justify-center">
          <Panel className="w-full max-w-xs p-6 rounded-2xl flex flex-col items-center text-center gap-4 bg-[var(--mb-surface-2)] border-2 border-black shadow-[var(--mb-shadow-lg)] relative overflow-hidden">
            <span className="text-xs font-black uppercase tracking-widest text-[var(--mb-text-dim)] [font-family:var(--mb-font-display)]">
              {t("games.eightstorm.ui.turn_label")}
            </span>

            <div className="flex items-center gap-3">
              <div className="relative">
                <WhimsicalAvatarFace avatarId={activeSeat?.avatarId ?? "fox"} size={52} />
                <span className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-[var(--mb-accent-2)] border-2 border-black" />
              </div>
              <span className="text-2xl font-black text-[var(--mb-text)] [font-family:var(--mb-font-display)]">{activeName}</span>
            </div>

            {pub.lastPlay ? (
              <div className="p-3 rounded-xl bg-[var(--mb-surface)] border-2 border-black shadow-[var(--mb-shadow)] w-full">
                <p className="text-sm font-extrabold text-[var(--mb-accent-2)]">
              ...
              <p className="text-xs font-bold text-[var(--mb-text-dim)] italic">
                Game underway!
              </p>
            )}
          </Panel>
          ...
          return (
            ...
            >
              <WhimsicalPlayerChip
                ...
                highlight={isTurn}
                trailing={
                  <div className="flex items-center gap-1 font-black text-sm px-2 py-0.5 rounded-md bg-[var(--mb-surface-3)] text-[var(--mb-gold)] border border-black">
                    <span>{count}</span>
      ...
      {isGameOver && winnerSeat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/80">
          <Card raised className="max-w-md w-full p-8 text-center flex flex-col items-center gap-5 border-4 border-black bg-[var(--mb-surface)] shadow-[var(--mb-shadow-lg)] mb-pop">
            <TrophyIcon className="w-20 h-20 text-[var(--mb-gold)] mb-tada" />
            ...
            <p className="text-xl font-extrabold text-[var(--mb-text)]">
              {t("games.eightstorm.phase.game_over")}
            </p>
          </Card>
        </div>
