...
- **Answer-key custody**: a `DailyContentPack.payload` contains the answer key. `init` splits it into `publicState` (what the player may see) and `secretState` (server-only); the key must never reach `publicState` before the attempt is over. This is the room-game `publicState` rule with a sharper edge — a leak does not spoil an advantage, it ends the puzzle.
- **Content authority**: puzzles enter through `POST /api/admin/daily/submit-pack`, gated by `DAILY_PIPELINE_SECRET` (fail-closed in production). The server calls that game's `validatePack` and stores the result as `queued` only when the submission carries `factCheck.status === "passed"`; everything else lands as `draft` and waits for an explicit decision on `/api/admin/daily/review/{id}/decide`. Rejecting a draft deletes it.
- **Writes overwrite**: `insertPack` upserts on `(game_id, puzzle_date)`. Re-submitting a date replaces that puzzle with no version history, so date selection is a correctness concern, not a convenience — use `scripts/daily-content.mjs`, which enforces it. Content authoring rules live in [docs/daily-content/](docs/daily-content/README.md).
